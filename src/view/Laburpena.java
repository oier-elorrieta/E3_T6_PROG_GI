...
public class Laburpena extends JFrame {
	...
	public Laburpena() {
		...
		for (int i = 0; i < SesioAldagaiak.sarreraKudeatzailea.getSarreraLista().size(); i++) {
			...
			JLabel lblFilmaIzena = new JLabel("Filma: " + SesioAldagaiak.sarreraKudeatzailea.getSarreraLista().get(i)
					.getSaioa().getFilma().getFilma_izena());
			...
			JLabel lblAretoaAtera = new JLabel("Aretoa: " + SesioAldagaiak.sarreraKudeatzailea.getSarreraLista().get(i)
					.getSaioa().getAretoa().getAreto_izena());
			...
			JLabel lblDataAtera = new JLabel("Data: "
					+ SesioAldagaiak.sarreraKudeatzailea.getSarreraLista().get(i).getSaioa().getData().getDate() + "/"
					+ (SesioAldagaiak.sarreraKudeatzailea.getSarreraLista().get(i).getSaioa().getData().getMonth() + 1)
					+ "/" + SesioAldagaiak.sarreraKudeatzailea.getSarreraLista().get(i).getSaioa().getData().getYear()
					+ " - " + View_metodoak.dateToString(
							SesioAldagaiak.sarreraKudeatzailea.getSarreraLista().get(i).getSaioa().getData()));
			...
			JLabel lblPetsonaKopAtera = new JLabel("Pertsona Kopurua: "
					+ SesioAldagaiak.sarreraKudeatzailea.getSarreraLista().get(i).getPertsonaKopurua());
		...
		JLabel lblPrezioTotala = new JLabel(
				"Prezio totala: " + SesioAldagaiak.sarreraKudeatzailea.getTicket_prezioa() + "€");
		...
		btnJarraitu.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (SesioAldagaiak.logeatuta) {
					int aukeraTicket = JOptionPane.showConfirmDialog(null, "Ticketa deskargatu nahi duzu?",
							"Ticket atera", JOptionPane.YES_NO_OPTION);

					if (aukeraTicket == JOptionPane.YES_OPTION) {
						try {
							View_metodoak.ticketSortu();
							JOptionPane.showMessageDialog(null, "Datuak ondo gorde dira", "Errorea", JOptionPane.INFORMATION_MESSAGE);
							dispose();
							JFrameSortu.zinemaAukera();
						} catch (IOException e1) {
							e1.printStackTrace();
						}
					} else if (aukeraTicket == JOptionPane.NO_OPTION) {
						JOptionPane.showMessageDialog(null, "Datuak ondo gorde dira", "Errorea", JOptionPane.INFORMATION_MESSAGE);
						dispose();
						JFrameSortu.zinemaAukera();
					}
				} else {
					JOptionPane.showMessageDialog(null, "Logeatu egin behar zara lehenengo!", "Errorea",
